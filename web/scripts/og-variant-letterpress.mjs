// og-variant-letterpress.mjs — 「昭和の実話怪談ムック／活版印刷」方向の OG 試作。
// 本番ルートと og-sample.mjs は改変しない。これは独立した検証用スクリプト。
// 狙い: AIテンプレ感(中央寄せ・均等3分割・丸ピル・放射グラデ)を捨て、
//        匡郭(版面の罫枠)・柱/ノンブル・非対称の版組で「古い怪談本の頁」に見せる。
import { ImageResponse } from "next/og.js";
import { createElement as h } from "react";
import { writeFileSync } from "node:fs";

// --- 配色: 焼けた紙(暖色のボーン) + 温かい墨 + 一点の血のような赤錆 ---
...
// 紙のテクスチャ: 隅のシミ(不穏) + 赤錆のにじみ + 簾の目(laid lines)。やりすぎない。
const paperBg = [
  "linear-gradient(135deg, rgba(20,15,10,0.18) 0%, rgba(20,15,10,0) 24%)",
  "linear-gradient(315deg, rgba(50,28,16,0.22) 0%, rgba(50,28,16,0) 30%)",
  "radial-gradient(150% 130% at 16% 86%, rgba(120,22,12,0.12) 0%, rgba(120,22,12,0) 38%)",
...
const tree = h("div", {
  ...
  h("div", {
    ...
    style: {
      ...
      margin: 34, padding: 6, border: `1.5px solid ${INK}`,
    },
  }, [
    h("div", {
      key: "block",
      style: {
        display: "flex", flexDirection: "column", flexGrow: 1,
        border: `1px solid ${INK}`, padding: "30px 52px",
      ...
      h("div", {
        ...
        h("div", {
          ...
          h("div", { key: "kick", style: { display: "flex", alignItems: "center", marginBottom: 18 } }, [
            h("div", { key: "kmark", style: { display: "flex", width: 14, height: 14, background: RED, marginRight: 16 } }),
        ...
        h("div", {
          ...
          h("div", { key: "rtop", style: { display: "flex", flexDirection: "column", alignItems: "flex-end" } }, [
            h("div", { key: "rbar", style: { display: "flex", width: 4, height: 44, background: RED, marginBottom: 22 } }),
          ...
          h("div", {
            key: "stamp",
            style: { display: "flex", fontSize: 20, letterSpacing: 6, color: INK, border: `1px solid ${INK}`, padding: "8px 14px" },
          }, STAMP),
        ]),
      ]),
      ...
      h("div", { key: "foot", style: { display: "flex", flexDirection: "column" } }, [
        ...
        h("div", {
          ...
          h("div", { key: "tags", style: { display: "flex", alignItems: "center" } }, [
            h("div", { key: "tbar", style: { display: "flex", width: 22, height: 2, background: RED, marginRight: 16 } }),
            h("div", { key: "ttxt", style: { display: "flex", fontSize: 24, letterSpacing: 3, color: INK } }, FOOT_L),
          ]),
...
const out = "/Users/kohei/.openclaw/workspace/yotogi_variant_letterpress_2026-06-22.png";
